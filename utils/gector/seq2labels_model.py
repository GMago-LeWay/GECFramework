...
from utils.gector.seq2labels_metric import Seq2LabelsMetric
...
class Seq2Labels(Model):
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        if self.metric is not None and not self.training:
            if reset:
                labels_accuracy = float(metrics_to_return['labels_accuracy'])
                print('The accuracy of predicting for edit labels is: ' + str(labels_accuracy))
                labels_accuracy_except_keep = float(metrics_to_return['labels_accuracy_except_keep'])
