...
class Seq2LabelsMetric(Metric):
    ...
    def get_metric(self, reset: bool = False, model_object=None):
        ret = {"labels_accuracy": float(self.Labels_Accuracy), "d_tags_accuracy": float(self.Tags_Accuracy), "labels_accuracy_except_keep": float(self.Labels_Accuracy_Except_Keep), "total_accuracy": float(self.Total_Accuracy)}
        if reset:
            self.reset()
        return ret
