...
from utils.gector.datareader import Seq2LabelsDatasetReader
from allennlp.common.model_card import Dataset
from allennlp.data import DataLoader
from allennlp.data import allennlp_collate

from utils.gector.helpers import write_lines, read_parallel_lines, encode_verb_form, \
...
def convert_data_from_data_list(data_list, output_file, vocab_path, min_count, save_vocab = False, worker_num = 8):
    tagged = []
    dic = defaultdict(int)  # 统计一下当前数据集里的编辑label及其出现次数，过滤掉出现次数过少的
    source_data = [item["text"] for item in data_list]
    target_data = [item["label"] for item in data_list]
    print(f"The size of raw dataset is {len(source_data)}")
    with Pool(worker_num) as pool:
        for aligned_sent, align_labels in pool.imap(align_sequences, tqdm(zip(source_data, target_data)), chunksize=8):
            if aligned_sent and align_labels:
